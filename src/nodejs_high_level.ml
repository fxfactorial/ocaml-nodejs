...
module Buffer = struct
  ...
  class buffer ?(default = 1024) fresh_buffer =
    object
...
module Events = struct
  class event_emmiter =
    object
      ...
      method event_names =
        k##eventNames |> Js.str_array |> Js.to_array |> Array.map Js.to_string
        |> Array.to_list

      method add_listener : 'a 'b. string -> ('a -> 'b) -> unit =
        fun event_name callback ->
...
module Fs = struct
  ...
  let read_file_sync file_name file_flag =
    let result =
      Nodejs.Fs.fs##readFileSync (Js.string file_name)
        (object%js
...
module Net = struct
  ...
  class socket fresh_socket =
    object
      ...
      initializer
        if fresh_socket then
          socket_handle <- new%js Nodejs.Net.socket |> Js.Opt.return;
        init_socket <- true
      ...
      method write data =
        ...
        else raise Socket_not_init
    end

  class server listener =
    object
      val k =
        let wrapped given_socket =
...
module Os = struct
  ...
  (* Hide as well in mli *)
  let endianess_of_string = function
    | "BE" -> Big
    | "LE" -> Little
    | _ -> assert false
  ...
  let platform_of_string = function
    ...
    | "darwin" -> Darwin
    | "freebsd" -> Freebsd
    | "linux" -> Linux (* TODO FINISH*)
    | _ -> assert false

  let cpu_stats () =
    os_handle##cpus |> Js.to_array
    |> Array.map (fun c ->
